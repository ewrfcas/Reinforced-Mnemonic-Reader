...
if __name__ == '__main__':
    ...
    def batchify_train(data):
        ...
        # load elmo
        with h5py.File(data_source + '/train_ELMO_feats.h5', 'r') as elmo_h5f:
            # with h5py.File(data_source + '/train_COVE_feats.h5', 'r') as cove_h5f:
                with h5py.File(data_source + '/train_data.h5', 'r') as h5f:
                    for qid in data:
                        group = h5f[str(qid)]
                        # base feats
        ...
        cont_feat = padding(cont_feat)
        ques_feat = padding(ques_feat)
        ...
        return cont_ids, ques_ids, cont_char_ids, ques_char_ids, cont_feat, ques_feat, elmo_cont_feat, elmo_ques_feat, y1s, y2s
    ...
    def batchify_dev(data):
        ...
        # cove_cont_feat, cove_ques_feat = [], []
        # load elmo
        with h5py.File(data_source + '/dev_ELMO_feats.h5', 'r') as elmo_h5f:
            # with h5py.File(data_source + '/dev_COVE_feats.h5', 'r') as cove_h5f:
                with h5py.File(data_source + '/dev_data.h5', 'r') as h5f:
                    for qid in data:
                        group = h5f[str(qid)]
                        # base feats
                        cont_ids.append(group['context_ids'][:])
    ...
    train_data_iter = gdata.DataLoader(train_qid, batch_size=config['batch_size'], last_batch='rollover',
                                       shuffle=True, batchify_fn=batchify_train, num_workers=8)
    dev_data_iter = gdata.DataLoader(dev_qid, batch_size=config['batch_size'], last_batch='keep',
                                     shuffle=False, batchify_fn=batchify_dev, num_workers=8)
    ...
    with tf.Session(config=sess_config) as sess:
        ...
        for i_epoch in range(config['epoch']):
            train_n_batch = len(train_data_iter)
            val_n_batch = len(dev_data_iter)
            # if (i_epoch + 1) % 8 == 0:
            ...
            for i_batch, batch_data in enumerate(train_data_iter):
                ...
                feed_dict_ = {model.contw_input: context_idxs, model.quesw_input: ques_idxs,
                              ...
                              model.cont_feat: context_feat, model.ques_feat: ques_feat,
            ...
            y1s = []
            y2s = []
            for i_batch, batch_data in enumerate(dev_data_iter):
                context_idxs, ques_idxs, \
                context_char_idxs, ques_char_idxs, \
                context_feat, ques_feat, \
                elmo_context_feat, elmo_question_feat, \
            ...
            logger.warning("-loss: %.4f -EM:%.2f%% (best: %.2f%%), -F1:%.2f%% (best: %.2f%%) -Noanswer:%d" %
                           (sum_loss_val / (val_n_batch + 1), metrics['exact_match'], best_em, metrics['f1'],
